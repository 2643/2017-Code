...
public class PracticeRobot extends IterativeRobot {
	...
	Talon lFrontMotor = new Talon(RandomPort);
	Talon lBackMotor = new Talon(RandomPort);
	Talon rFrontMotor = new Talon(RandomPort);
	Talon rBackMotor = new Talon(RandomPort);
	...
	public void drive(){
		...
		if(stick.getRawButton(arcadeOn))
		{
			isArcadeOn = true;
		}

		else if(stick.getRawButton(arcadeOff))
		{
			isArcadeOn = false;
		}
	...
	public void toggleDrive() {
		...
		//if arcade drive is not on, then use the drive toggle for tank drive
		if(!isArcadeOn)
		{
		...
		//if arcade drive is on, then use the drive toggle for arcade drive
		else if(isArcadeOn)
		{
			if(!driveToggle)
			{
			...
			{
				myDrive.arcadeDrive(stick.getY()*0.5, stick.getZ()*0.5, true);
			}
		}
	}
}
